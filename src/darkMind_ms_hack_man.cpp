...
public:
    ...
    bool operator<(const SimpleObject &so) const
    { //used to sort the snippets vector from the closest to the farthest
        return euclidianDistanceCells(darkMind.x, darkMind.y, this->x, this->y)
                < euclidianDistanceCells(darkMind.x, darkMind.y, so.x, so.y);
    }
};
...
public:
    ...
    bool operator<(const ObjectWithRounds &a) const
    {
...
void parseObjects(const string & objList, const int &cell)
{
    ...
    else
    {
        ...
        std::vector<string> objs((std::istream_iterator<StringDelimitedBy<';'>>(issl)),
...
void do_move()
{
    map<ObjectWithRounds,vector<int> >mappa = checkDangerNode();
    if(!mappa.empty())
    {
        map<ObjectWithRounds, vector<int> >::iterator it=mappa.begin();
        while(it!=mappa.end())
        {
            for(int i=0;i<it->second.size();i++)
                cerr<<"Bomba "<<it->first.node<<"   scade tra "<<it->first.rounds<<"   "<<it->second[i]<<endl;
...
string pathFind(bool** &adiacents, const int & posBegin, const int & posEnd)
{
    ...
    while (!pq[pqi].empty())
    {
        n0 = new Point(pq[pqi].top().getX(), pq[pqi].top().getY(), pq[pqi].top().getLevel(),
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            if (!(xdx < 0 || xdx > numNodes - 1 || ydy < 0 || ydy > numNodes - 1 || closed_nodes_map[xdx][ydy] == 1)
                    && (nodeStart >= 0 && nodeStart < numNodes) && (nodeNext >= 0 && nodeNext < numNodes)
                    && adiacents[nodeStart][nodeNext])
            {
...
map<ObjectWithRounds, vector<int> > checkDangerNode()
{
    ...
    for(int i=0;i<weapons.size();i++)
        if(weapons[i].rounds != -1)
        dangers[weapons[i]] = getDangerNodes(weapons[i]);
    return dangers;
}
...
vector<int> getDangerNodes(ObjectWithRounds bomb)
{
    ...
    for(int i=y;i<height-1;i++)
    {
        ...
        if(matrixAdiacents[currentNode][nextNode] == 0)
            break;
